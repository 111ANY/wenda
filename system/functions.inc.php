...
function base_url()
{
	$clean_url = isset($_SERVER['PATH_INFO']) ? $_SERVER['PATH_INFO'] : NULL;//isset() 判断检测变量是否设置，并且不是 NULL//a?b:c;等于if(a)则b否则c
	                                                                      //PATH_INFO  是  www.test.com/index.php/foo/bar.html?c=index&m=search中的/foo/bar.html
																   //QUERY_STRING  是  c=index&m=search
	$clean_url = dirname(rtrim($_SERVER['PHP_SELF'], $clean_url));//dirname() 函数返回路径中的目录部分
	$clean_url = rtrim($_SERVER['HTTP_HOST'] . $clean_url, '/\\');//rtrim(string<必选>,charlist<可选>) 函数移除字符串右侧的空白字符或其他预定义字符
